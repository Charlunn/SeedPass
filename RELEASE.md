...
- 填充页默认折叠低频的密码规则和已保存记录列表。
- 当前网站、账号标识、备注、生成方式和主要填充按钮默认保留在首屏。
- 生成方式、按钮组和锁定按钮进一步紧凑化。
- 创建页、解锁页、填充页统一按首屏完成主要操作的目标压缩布局。
...
- 首次使用说明改为默认折叠，减少创建页占用。
- 创建页 PIN 设置和确认改为并排显示。
- 填充页主要操作按钮改为紧凑按钮组。
- 缩小表单间距、控件高度和记录列表高度，让常用操作尽量在一屏内完成。
...
- 创建保险库时明确区分“我已有，自己输入”和“帮我生成一组新的”。
- 默认隐藏生成助记词按钮，避免用户误以为必须点击生成。
- 增加首次使用说明卡片，解释基密码、PIN 和保存流程。
- 首次创建时默认聚焦基密码输入框，而不是 PIN 输入框。
...
- 统一 popup 的颜色、圆角、间距、按钮、输入框、PIN 框和记录列表样式。
- 统一页面内填充浮层和 popup 的视觉风格。
- 改善小宽度窗口下的缩放与排列。
- 长网站名、长备注和多条记录显示更稳定。
- 保留 `chrome-edge-v0.4.0` 的记录和生成方式逻辑。
